...
ckpt_path_tp = "E:/Trap-MID/grad_cam/VGG16_82.01_99.93_allclass.tar"  # with trapdoor behavior weights
ckpt_tp = torch.load(ckpt_path_tp, map_location="cpu")
trigger_path = "E:/Trap-MID/grad_cam/trigger.tar" # trigger for VGG-16
...
α = 0.02                                  
img_clean = cv2.imread("E:/Trap-MID/grad_cam/ked_mi/origin.png")
...
target_layer = tp_model.feature[-1]          # VGG-16 最後卷積

cam = GradCAM(tp_model, [target_layer])

hm_clean, g_clean = cam_img(img_clean, cam)
hm_trap , g_trap  = cam_img(img_trap, cam)
hm_mi   , g_mi    = cam_img(cv2.imread("E:/Trap-MID/grad_cam/ked_mi/attack_with_trap.png"), cam)

cv2.imwrite("cam_clean.jpg", hm_clean)
cv2.imwrite("cam_trap.jpg" , hm_trap )
cv2.imwrite("cam_mi.jpg"   , hm_mi   )
...
ckpt_path_bk = "E:/Trap-MID/grad_cam/VGG16_88.26.tar"      # without trapdoor behavior weights
...
target_layer = bk_model.feature[-1]          # VGG-16 最後卷積

cam = GradCAM(bk_model, [target_layer])

hm_clean, g_clean = cam_img(img_clean, cam)
hm_trap , g_trap  = cam_img(img_trap, cam)
hm_mi   , g_mi    = cam_img(cv2.imread("E:/Trap-MID/grad_cam/ked_mi/attack_without_trap.png"), cam)
